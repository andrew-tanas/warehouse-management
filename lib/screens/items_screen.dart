...
class _ItemsScreenState extends State<ItemsScreen> {
    ...
    showDialog(
      ...
      builder: (context) {
        return AlertDialog(
          ...
          title: Text(item == null ? AppLocalizations.of(context).translate('add_item') : AppLocalizations.of(context).translate('edit'), style: const TextStyle(fontWeight: FontWeight.bold)),
          content: Form(
            key: formKey,
                ...
                child: Column(
                  ...
                  children: [
                      ...
                      children: [
                        Expanded(
                          child: TextFormField(
                            ...
                            decoration: InputDecoration(
                              labelText: AppLocalizations.of(context).translate('retail_price'),
                              border: OutlineInputBorder(borderRadius: BorderRadius.circular(12)),
                              filled: true,
                              fillColor: Colors.grey.shade50,
                            ),
                            ...
                            onSaved: (val) => retailPrice = double.tryParse(val!) ?? 0.0,
                          ),
                        ),
                        ...
                        Expanded(
                          child: TextFormField(
                            ...
                            decoration: InputDecoration(
                              labelText: AppLocalizations.of(context).translate('wholesale_price'),
                              border: OutlineInputBorder(borderRadius: BorderRadius.circular(12)),
                              filled: true,
                              fillColor: Colors.grey.shade50,
                            ),
                            keyboardType: const TextInputType.numberWithOptions(decimal: true),
                            inputFormatters: [FilteringTextInputFormatter.allow(RegExp(r'^\d+\.?\d*'))],
                            onSaved: (val) => wholesalePrice = double.tryParse(val!) ?? 0.0,
                          ),
                        ),
                      ],
                    ),
                    ...
                    TextFormField(
                      ...
                      decoration: InputDecoration(
                        labelText: AppLocalizations.of(context).translate('custom_price'),
                        border: OutlineInputBorder(borderRadius: BorderRadius.circular(12)),
                        filled: true,
                        fillColor: Colors.grey.shade50,
                      ),
                      keyboardType: const TextInputType.numberWithOptions(decimal: true),
                      inputFormatters: [FilteringTextInputFormatter.allow(RegExp(r'^\d+\.?\d*'))],
                      onSaved: (val) => customPrice = (val == null || val.isEmpty) ? null : double.tryParse(val),
                    ),
                    const SizedBox(height: 16),
                    TextFormField(
                      initialValue: stockAmount.toString(),
                      decoration: InputDecoration(
                        labelText: AppLocalizations.of(context).translate('stock_amount'),
                        border: OutlineInputBorder(borderRadius: BorderRadius.circular(12)),
                        filled: true,
                        fillColor: Colors.grey.shade50,
                      ),
                      keyboardType: const TextInputType.numberWithOptions(decimal: true),
                      inputFormatters: [FilteringTextInputFormatter.allow(RegExp(r'^\d+\.?\d*'))],
                      onSaved: (val) => stockAmount = double.tryParse(val!) ?? 0.0,
                    ),
                  ],
                ),
              ),
              ...
              onPressed: () {
                if (formKey.currentState!.validate()) {
                  ...
                  if (item == null) {
                    ...
                    Provider.of<ItemProvider>(context, listen: false).updateItem(newItem);
                  }
                ...
                ElevatedButton(
                  onPressed: () {
                    if (formKey.currentState!.validate()) {
                      ...
                      Provider.of<ItemProvider>(context, listen: false).updateStock(
                        item.id!, 
                        change,
                        date: selectedDate,
                        note: note.isEmpty ? (isAdding ? 'Added manually' : 'Reduced manually') : note,
                      );
  ...
  Widget _buildPriceCell(Item item, String priceType, double? price, Color color) {
    ...
    return InkWell(
      ...
      child: Text(price != null ? price.toStringAsFixed(2) : '-', style: TextStyle(color: color, fontWeight: FontWeight.bold)),
    );
  }
  ...
  void _updateInlinePrice(Item item, String priceType, String val) {
    ...
    final updatedItem = Item(
      ...
      customPrice: priceType == 'custom' ? (val.isEmpty ? null : newPrice) : item.customPrice,
            ...
            return AlertDialog(
                  ...
                  children: [
                    Autocomplete<Item>(
                      ...
                      fieldViewBuilder: (context, textEditingController, focusNode, onFieldSubmitted) {
                        ...
                        return TextFormField(
                          ...
                          focusNode: focusNode,
                          decoration: InputDecoration(
                            labelText: AppLocalizations.of(context).translate('select_item') + ' (Optional)',
                            border: OutlineInputBorder(borderRadius: BorderRadius.circular(12)),
                            filled: true,
                            fillColor: Colors.grey.shade50,
                        ...
                        child: Wrap(
                          ...
                          children: selectedItemsForPrint.map((item) {
                            return Chip(
                              ...
                              onDeleted: () {
                                setState(() {
                                  selectedItemsForPrint.removeWhere((i) => i.id == item.id);
                                });
                              },
                            );
                          }).toList(),
                    ...
                    ...availableColumns.map((col) {
                      return CheckboxListTile(
                        title: Text(col == 'id' ? 'ID' : AppLocalizations.of(context).translate(col)),
                        ...
                        onChanged: (bool? value) {
                          setState(() {
                            if (value == true) {
                              ...
                              selectedColumns.sort((a, b) => availableColumns.indexOf(a).compareTo(availableColumns.indexOf(b)));
                            } else {
              ...
              actions: [
                ...
                ElevatedButton(
                  onPressed: () async {
                    Navigator.of(context).pop();
                    List<Item> finalItemsToPrint = selectedItemsForPrint.isNotEmpty ? selectedItemsForPrint : items;
                    await PdfGenerator.printItemsList(context, finalItemsToPrint, selectedColumns);
                  },
  ...
  Widget build(BuildContext context) {
    return AppNavigation(
      ...
      appBar: AppBar(
        title: Text(AppLocalizations.of(context).translate('items_management'), style: const TextStyle(fontWeight: FontWeight.w600)),
        elevation: 0,
                  ...
                  child: DropdownButtonHideUnderline(
                    child: DropdownButton<int>(
                      ...
                      items: [
                        ...
                        DropdownMenuItem(value: 1, child: Text(AppLocalizations.of(context).translate('name'))),
                        DropdownMenuItem(value: 2, child: Text(AppLocalizations.of(context).translate('size'))),
                        DropdownMenuItem(value: 3, child: Text(AppLocalizations.of(context).translate('retail_price'))),
                        DropdownMenuItem(value: 4, child: Text(AppLocalizations.of(context).translate('wholesale_price'))),
                        DropdownMenuItem(value: 5, child: Text(AppLocalizations.of(context).translate('custom_price'))),
                        DropdownMenuItem(value: 6, child: Text(AppLocalizations.of(context).translate('stock_amount'))),
                      ],
                  ...
                  builder: (context, provider, child) {
                    ...
                    if (_sortColumnIndex != null) {
                      itemsToPrint.sort((a, b) {
                        ...
                        switch (_sortColumnIndex) {
                          ...
                          case 3: cmp = a.retailPrice.compareTo(b.retailPrice); break;
                          case 4: cmp = a.wholesalePrice.compareTo(b.wholesalePrice); break;
                          case 5: cmp = (a.customPrice ?? 0).compareTo(b.customPrice ?? 0); break;
                          case 6: cmp = a.stockAmount.compareTo(b.stockAmount); break;
                        }
                    ...
                    return Container(
                      ...
                      child: IconButton(
                        ...
                        onPressed: itemsToPrint.isEmpty ? null : () => _showPrintDialog(itemsToPrint),
            ...
            child: Consumer<ItemProvider>(
              ...
              child: Column(
                ...
                children: [
                  ...
                  const SizedBox(height: 16),
                  Text(AppLocalizations.of(context).translate('no_items'), style: TextStyle(fontSize: 18, color: Colors.grey.shade600)),
                ],
              )
            );
          }
                  ...
                  child: SizedBox(
                    ...
                    child: DataTable(
                      ...
                      rows: filteredItems.map((item) {
                        return DataRow(
                          onSelectChanged: (_) {
                            Navigator.push(context, MaterialPageRoute(builder: (_) => ItemHistoryScreen(item: item)));
                          },
                          cells: [
                            DataCell(Text(item.id.toString())),
                            DataCell(Text(item.name, style: const TextStyle(fontWeight: FontWeight.w500))),
                            DataCell(Text(item.size)),
                            DataCell(_buildPriceCell(item, 'retail', item.retailPrice, Colors.green)),
                            DataCell(_buildPriceCell(item, 'wholesale', item.wholesalePrice, Colors.blue)),
                            DataCell(_buildPriceCell(item, 'custom', item.customPrice, Colors.orange)),
                              ...
                              Row(
                                ...
                                children: [
                                  ...
                                  IconButton(
                                    ...
                                    onPressed: () => _showStockDialog(item, false),
                                  ),
                                  IconButton(
